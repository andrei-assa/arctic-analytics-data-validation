...
class NonUniqueValueError(Exception):
    ...
    def __init__(
        self, column_name, message="Values in the column should be unique."
    ):
...
class NonNullError(Exception):
    ...
    def __init__(
        self,
        column_name,
        message="Some value in the column is null. Please fill in missing values.",
    ):
        self.column_name = column_name
        self.message = message
        super().__init__(self.message)
...
class DataValidator(object):
    def __init__(
        ...
        *,
        ...
        self.is_valid = False
        self.data = data
        self._order_id_col_name = (
            "order_id" if order_id_col_name is None else order_id_col_name
        )
        self._product_id_col_name = (
            "product_id"
            if product_id_col_name is None
            else product_id_col_name
        )
        ...
        product_ids_as_lists = list(
            self.data[self._product_id_col_name].apply(_check_list_like)
        )
        ...
        # Long format data: (1) non-unique order id's
            ...
            print(
                "Data is of unknown format. Please check your inputs."
            )  # logger
        ...
        if self.is_valid:
            ...
            order_ids_correct_data_type = all(
                [
                    isinstance(x, (int, str))
                    for x in self.data[self._order_id_col_name]
                ]
            )

            # Check that product id's are of correct data type (int, str).
            product_ids_correct_data_type = all(
                [
                    isinstance(x, (int, str))
                    for x in self.data[self._product_id_col_name]
                ]
            )
